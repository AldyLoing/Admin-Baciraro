...
export default function Sidebar({ name, role, isAdmin, avatarUrl }: SidebarProps) {
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({
    Finance: accountingItems.some((a) => pathname.startsWith(a.href)),
  });
  ...
  const isFinanceActive = accountingItems.some((a) => pathname.startsWith(a.href));
      ...
      >
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold leading-tight">Baciraro</p>
                <p className="text-xs text-white/50">Operating System</p>
              </div>
            </div>
            <button
              onClick={() => setMobileOpen(false)}
        ...
        <nav className="flex-1 p-4 space-y-4 overflow-y-auto">
          {navSections.map((section) => {
            const isFinance = section.title === "Finance";
            const sectionOpen = isFinance ? (openSections["Finance"] ?? false) : true;

            return (
              <div key={section.title}>
                <p className="px-4 mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-white/30">
                  {section.title}
                </p>
                <div className="space-y-0.5">
                  ...
                  {/* Finance section with collapsible Accounting */}
                  {isFinance && (
                    <>
                      <button
                        onClick={() => toggleSection("Finance")}
                        className={cn(
                          "flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-medium transition w-full text-left",
                          isFinanceActive && !section.items.some((i) => pathname.startsWith(i.href))
                            ? "bg-gradient-to-r from-[#C44A3A] to-[#D97A2B] text-white shadow-lg shadow-orange-500/20"
                            : "text-white/60 hover:bg-white/5 hover:text-white"
                        )}
                      >
